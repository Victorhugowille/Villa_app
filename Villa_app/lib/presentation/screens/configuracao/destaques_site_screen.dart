...
class _DestaquesSiteScreenState extends State<DestaquesSiteScreen> {
  ...
  Map<int, app_data.DestaqueSite> _destaquesMap = {};
  ...
  Future<void> _fetchDestaques() async {
    ...
    try {
      ...
      final destaques =
          response.map((item) => app_data.DestaqueSite.fromJson(item)).toList();
  ...
  Widget build(BuildContext context) {
    final images = _destaquesMap.values.map((d) => d.imageUrl).whereType<String>().toList();
    images.sort((a, b) {
  ...
  Widget _buildSlotEditor(int slotNumber, app_data.DestaqueSite? destaque) {
    return AspectRatio(
      aspectRatio: 1.0,
